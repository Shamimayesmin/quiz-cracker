...
const Quiz = ({ques,index}) => {
	...
	return (
		<div className=" border mt-10 bg-slate-300 rounded-md">
				...
				<p className="mt-8">Quit- {index+1} :{question.slice(3,-4)}</p>
				<button onClick={handleIcon}><EyeIcon  className="h-6 w-6 mx-3 mt-5"/></button>
				 <div className="m-6 p-10 rounded-md bg-slate-200 grid lg:grid-cols-2 gap-8 sm:grid-cols-1">
					{
						options.map((option) =><label key={option.id} className="preference mx-auto p-8 w-64 bg-teal-500  py-8 rounded-lg " >
							<input onClick={() =>handleClickBtn(option)} type="radio" name={index} value="correctAnswer"  />{option}</label>)
					}
				 </div>
		</div>
	);
